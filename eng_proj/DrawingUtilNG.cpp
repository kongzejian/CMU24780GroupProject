...
void DrawingUtilNG::drawTiltRect(double x, double y, double angle, double sizeX, double sizeY, bool filled)
{
	double diagonal = sqrt(pow(sizeX,2) + pow(sizeY,2 ));

	if (filled)
		glBegin(GL_QUADS);
	else
		glBegin(GL_LINE_LOOP);
	double vert2X = x + sizeY * sin(angle * 3.14 / 180.);
	double vert2Y = y + cos(angle * 3.14 / 180.) * sizeY;
	double vert3X = vert2X + cos(angle * 3.14 / 180) * sizeX;
	double vert3Y = vert2Y - sin(angle * 3.14 / 180) * sizeX;
	double vert4X = x + +cos(angle * 3.14 / 180) * sizeX;
	double vert4Y = y - sin(angle * 3.14 / 180) * sizeX;
	glVertex2d(x, y);
...
void DrawingUtilNG::drawIsoscelesTriangle(double x1, double y1, double x2, double y2, double height, bool filled)
{
	double midPointX = (x1 + x2) / 2;
	double midPointY = (y1 + y2) / 2;
	double angleIncline = atan2((y2 - y1), (x2 - x1));
	//std::cout << angleIncline << std::endl;
	double vert2X = x2;
	double vert2Y = y2;
	double vert3X = midPointX + height * sin(angleIncline);
	double vert3Y = midPointY - height * cos(angleIncline);
